...
L.ShowWAKeys						= "Показывать ключи WeakAuras рядом с именами заклинаний, чтобы помочь в написании WeakAuras с использованием триггеров Boss Mod. Нюанс заключается в том, что это не будет показывать правильный ключ в битвах фракций, идентификаторы которых могут меняться при пуллах в зависимости от фракции лидера группы"
...
L.Area_SpamFilter_Nameplates= "Глобальное отключение индакаторов и параметры фильтрации"
L.SpamBlockNoNameplate		= "Не показывать ауры индикаторов (отключает полностью)"
L.SpamBlockNoBossGUIDs		= "Не показывать таймеры главного босса (босс 1) на индикаторах Plater в качестве аур индикаторов\n(Вы все равно будете видеть таймеры трэша/босса, если эта функция включена в Plater)"
L.SpamBlockTimersWithNameplates	= "Не показывать полоски таймера для трэша, если в настройках Plater включено 'восстановление способностей аур индикаторов' (не относится к боям с боссами, которые всегда будут показывать полоски таймера)"
L.NameplateFooter			= "Здесь доступны дополнительные функции, если включено 'индикаторы Plater'"
